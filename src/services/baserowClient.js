...
if (!BASEROW_API_URL || !BASEROW_API_TOKEN || !TIER_ACTIVATIONS_TABLE_ID) {
  throw new Error(
    "Missing required environment variables: BASEROW_API_URL, BASEROW_API_TOKEN, BASEROW_TIER_ACTIVATIONS_TABLE_ID"
  );
}
...
async function fetchRecentTierActivations(sinceIso) {
  const filter = sinceIso
    ? `&filter__timestamp__gt=${encodeURIComponent(sinceIso)}`
    : "";
